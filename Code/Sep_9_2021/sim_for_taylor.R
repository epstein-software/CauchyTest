...
source('DKAT_source.R') # source code for the DKAT method
source('SMAT_code.R') # source code for scaled marginal model
source('source_files_for_taylor.R') # source code for remaining functions
...
sim_z_bin<-rbinom(Nsub,1,p_z_bin) # generates binary environmental covariate
sim_z_con<-rnorm(Nsub,0,1)
#sim_g<-rbinom(Nsub,2,maf_0*(sim_z_bin==0)+maf_1*(sim_z_bin==1))
sim_g<-rbinom(Nsub,2,maf) # generate SNP genotype that is not dependent on binary covariate


preds2<-rbind(sim_g,sim_z_bin,sim_z_con)
preds3<-split(preds2,rep(1:ncol(preds2),each=nrow(preds2)))

#Generate expression data

sim_y<-t(sapply(preds3,gen_express_cov,int=int,main_beta_g=main_beta_g,var_beta_g=var_beta_g,Num_off_diag=Num_off_diag,cov_beta_g=cov_beta_g,sigma_e=sigma_e,main_beta_z_bin=main_beta_z_bin,main_beta_z_con=main_beta_z_con,var_beta_z_bin=var_beta_z_bin,var_beta_z_con=var_beta_z_con,cov_beta_z_bin=cov_beta_z_bin,cov_beta_z_con=cov_beta_z_con))

# Test for differential co-expression using the GAMuT framework
pval_GAMuT_adj_dglm[isim,]<-GAMuT_cor_cov_dglm(sim_y,sim_g,sim_z_con)
